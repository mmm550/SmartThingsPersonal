...
metadata {
	...
	tiles {
		standardTile("switch1", "device.switch", width: 2, height: 2, canChangeIcon: true, canChangeBackground: true) {
		...
		main "switch1"
		details(["switch1","greeting","message"])
	}
    simulator {
...
def on() {
    zigbee.smartShield(text: "on1").format()
}

def off() {
	zigbee.smartShield(text: "off1").format()
}
