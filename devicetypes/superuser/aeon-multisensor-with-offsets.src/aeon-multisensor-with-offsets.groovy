metadata {
	...
	definition (name: "Aeon Multisensor with Offsets", namespace: "", author: "Humac") {
    ...
    preferences {
        ...
        input "humOffset", "number", title: "Humidity Offset", description: "Adjust humidity by this many degrees", range: "*..*", displayDuringSetup: false
	}
...
def zwaveEvent(physicalgraph.zwave.commands.sensormultilevelv2.SensorMultilevelReport cmd)
{
	...
	switch (cmd.sensorType) {
		...
		case 5:
        	// humidity
        	def preValueHum = convertHumidityIfNeeded(cmd.scaledSensorValue, cmdScale, cmd.precision)
            def valueHum = preValueHum as float
            if (humOffset) {
            def offsetHum = humOffset as float
            map.valueHum = valueHum + humoffset as float
            map.valueHum = map.valueHum.round()
            }
            else {
            	map.valueHum = valueHum as float
                map.valueHum = map.valueHum.round()
            }
            map.unit = getHumidityScale()
			map.name = "humidity"
			break;
        	}
	map
